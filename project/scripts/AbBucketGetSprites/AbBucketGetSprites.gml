...
function AbBucketGetSprites(_bucketName, _outputArray = undefined)
{
    ...
    else
    {
        if (_outputArray == undefined)
        {
            _outputArray = variable_clone(_bucket.__spriteArray);
        }
        else
        {
            array_resize(_outputArray, 0);
            array_copy(_outputArray, 0, _bucket.__spriteArray, 0, array_length(_bucket.__spriteArray));
        }
    }
