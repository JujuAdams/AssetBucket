...
function __BucketLooseJSONRead(_buffer, _inOffset = undefined)
{
    ...
    while(buffer_tell(_buffer) < _bufferSize)
    {
        ...
        {
            __BucketLooseJSONReadComment(_buffer, _bufferSize);
        }
        else if (_byte == ord("["))
        {
            _result = __BucketLooseJSONReadArray(_buffer, _bufferSize);
        }
        else if (_byte == ord("{"))
        {
            _result = __BucketLooseJSONReadStruct(_buffer, _bufferSize);
        }
        else if (_byte > 0x20)
        {
...
function __BucketLooseJSONReadArray(_buffer, _bufferSize)
{
    var _result = [];
    ...
    while(buffer_tell(_buffer) < _bufferSize)
    {
        ...
        {
            var _value = __BucketLooseJSONReadValue(_buffer, _bufferSize, _byte);
...
function __BucketLooseJSONReadStruct(_buffer, _bufferSize)
{
    var _result = {};
    ...
    while(buffer_tell(_buffer) < _bufferSize)
    {
        ...
        {
            var _key = __BucketLooseJSONReadValue(_buffer, _bufferSize, _byte);
            ...
            else //Is an array
            {
                ...
                repeat(_keyArrayLength-1)
                {
                    ...
                    _result[$ _key] = __BucketLooseJSONReadDeepCopyInner(_value, self, self);
                    ++_i;
                }
            }
...
function __BucketLooseJSONReadValue(_buffer, _bufferSize, _firstByte)
{
    if (_firstByte == ord("["))
    {
        return __BucketLooseJSONReadArray(_buffer, _bufferSize);
    }
    else if (_firstByte == ord("{"))
    {
        return __BucketLooseJSONReadStruct(_buffer, _bufferSize);
    }
    else if (_firstByte == ord("\""))
    {
        return __BucketLooseJSONReadDelimitedString(_buffer, _bufferSize);
    }
    else if ((_firstByte == ord("@")) && (buffer_peek(_buffer, buffer_tell(_buffer), buffer_u8) == ord("'")))
    {
        buffer_seek(_buffer, buffer_seek_relative, 1);
        return __BucketLooseJSONReadMultilineString(_buffer, _bufferSize);
    }
    else
    {
        return __BucketLooseJSONReadString(_buffer, _bufferSize);
    }
}

function __BucketLooseJSONReadDelimitedString(_buffer, _bufferSize)
{
...
function __BucketLooseJSONReadMultilineString(_buffer, _bufferSize)
{
    static _cacheBuffer = buffer_create(1024, buffer_grow, 1);
    buffer_seek(_cacheBuffer, buffer_seek_start, 0);
    ...
    show_error("Found unterminated multiline string\n ", true);
}

function __BucketLooseJSONReadString(_buffer, _bufferSize)
{
    static _cacheBuffer = buffer_create(1024, buffer_grow, 1);
    buffer_seek(_cacheBuffer, buffer_seek_start, 0);
...
function __BucketLooseJSONReadComment(_buffer, _bufferSize)
{
    while(buffer_tell(_buffer) < _bufferSize)
    {
        var _byte = buffer_read(_buffer, buffer_u8);
...
function __BucketLooseJSONReadDeepCopyInner(_value, _oldStruct, _newStruct)
{
    var _copy = _value;
    ...
    if (is_method(_value))
    {
        ...
        else if (_self != undefined)
        {
            ...
            show_debug_message("DeepCopy(): Warning! Deep copy found a method reference that could not be appropriately handled");
        }
    }
    else if (is_struct(_value))
    {
        ...
        repeat(array_length(_namesArray))
        {
            ...
            _copy[$ _name] = __BucketLooseJSONReadDeepCopyInner(_value[$ _name], _value, _copy);
            ++_i;
        }
    }
    else if (is_array(_value))
    {
        ...
        repeat(_count)
        {
            _copy[@ _i] = __BucketLooseJSONReadDeepCopyInner(_value[_i], _oldStruct, _newStruct);
            ++_i;
        }
    }
