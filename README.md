...
Project assets must be added to the GameMaker project via the IDE, creating a copy of the asset within the project directory. For example, to add audio to your game you must add the audio file as a "sound asset" which causes GameMaker to create a copy of the source audio file in the project alongside some metadata (compression settings, mono/stereo, etc.). Manually adding files to the IDE is time-consuming. Many assets (backgrounds, 3D textures, most audio) are straight imports and don't need all the customisation options GameMaker provides. It is troublesome to try to keep assets up to date if there are multiple copies in different places as is the case when working with artists and composers who many never often the GameMaker IDE.
...
4. To make it easier to ingest files from disk, use `AbForeachFile()` or `AbForeachFileFiltered()` to iterate over particular directories
...
3. To make it easier to ingest files from disk, use `AbForeachFile()` or `AbForeachFileFiltered()` to iterate over particular directories
